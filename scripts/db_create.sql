...
-- 标准规则表（规则制定）：定义和存储标准的地址前缀列表配置模板
DROP TABLE IF EXISTS prefix_list_standards;
CREATE TABLE prefix_list_standards (
...
-- 设备实际配置表（规则记录）：存储从设备采集的实际地址前缀列表配置
DROP TABLE IF EXISTS prefix_list_records;
CREATE TABLE prefix_list_records (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
...
-- 地址前缀问题处理记录表：记录配置漂移和缺失问题的处理流程
DROP TABLE IF EXISTS prefix_list_issue_records;
CREATE TABLE prefix_list_issue_records (
...
-- 3. 问题处理：从管理配置创建问题记录到 prefix_list_issue_records，进行流程跟踪和工单管理
